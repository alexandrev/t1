/*
 * Copyright (c) 2008-2022 Spotfire AB,
...
create table "SN_VERSION" (
  ...
  "UPGRADE_INITIATED" datetime null,
  "UPGRADE_INFO" varchar(400) null )
...
create table "USERS" (
  ...
  "GIVEN_NAME" nvarchar(450) null,
  "FAMILY_NAME" nvarchar(450) null,  
...
create table "LIB_DATA" (
	...
	"CONTENT_SIZE" bigint not null constraint "LIB_DATA_CONTENT_SIZE_DF" default 0,
	"COMPRESSED_CONTENT_SIZE" bigint null,
	"DIGEST" varbinary(max) null,
...
create table "LIB_ITEM_VERSIONS" (
	...
	"VERSION_COMMENT" nvarchar(1000) null,
	"RESTORED_FROM_ID" char(36) null,
	"RESTORED_FROM_TIMESTAMP" datetime null,
...
create table "LIB_CURRENT_ITEMS" ( 
	...
	foreign key ("VERSION_ID") 
		references "LIB_ITEM_VERSIONS" ("VERSION_ID") on update no action on delete no action, 
	foreign key ("CREATED_BY") 
		references "USERS" ("USER_ID") on update set null on delete set null,
	foreign key ("MODIFIED_BY") 
		references "USERS" ("USER_ID") on update no action on delete no action,
	foreign key ("PARENT_ID") 
		references "LIB_CURRENT_ITEMS" ("ITEM_ID") on update no action on delete no action,
...
create view "LIB_ITEMS" as
  select 
	...
	liv.DATA_ID 
  from 
	LIB_CURRENT_ITEMS lci 
	left outer join LIB_ITEM_VERSIONS liv 
		on lci.VERSION_ID = liv.VERSION_ID
	left outer join LIB_DATA ld 
		on liv.DATA_ID = ld.DATA_ID

go

alter table "LIB_CURRENT_ITEMS" add constraint "LIB_CURRENT_ITEMS_PARENT_NEQ_ITEM" 
	check ("ITEM_ID" != "PARENT_ID")

go

create unique index "LIB_CURRENT_ITEMS_UIX1" on "LIB_CURRENT_ITEMS"
  ("TITLE", "ITEM_TYPE", "PARENT_ID");

go

create index "LIB_CURRENT_ITEMS_IX1" 
	on "LIB_CURRENT_ITEMS" ("CREATED_BY")  
go

create index "LIB_CURRENT_ITEMS_IX2" 
	on "LIB_CURRENT_ITEMS" ("MODIFIED_BY")  
go

create index "LIB_CURRENT_ITEMS_IX3" 
	on "LIB_CURRENT_ITEMS" ("ITEM_TYPE")  
go

create index "LIB_CURRENT_ITEMS_IX4" 
	on "LIB_CURRENT_ITEMS" ("TITLE")  
go

create index "LIB_CURRENT_ITEMS_IX5" 
	on "LIB_CURRENT_ITEMS" ("PARENT_ID")  
go

create index "LIB_CURRENT_ITEMS_IX6" 
	on "LIB_CURRENT_ITEMS" ("VERSION_ID", "ITEM_ID")  
go


/* Trigger instead of foreign key constraint due to MSSQL limitations ("recursive" triggers) */
create trigger "TR_LIB_CURRENT_ITEMS_USER" on "USERS" instead of delete 
	as
	update "LIB_CURRENT_ITEMS" set "MODIFIED_BY" = null where "MODIFIED_BY" in (select "USER_ID" from "DELETED");
...
create table "LIB_PROPERTIES_VER" ( 
	...
	constraint "FK_LIB_PROPERTIES_VER_FK1" foreign key ("VERSION_ID")
	 	references "LIB_ITEM_VERSIONS" ("VERSION_ID") on update no action on delete cascade
)

go

create index "LIB_PROPERTIES_VER_IX1" on "LIB_PROPERTIES_VER" ("VERSION_ID", "PROPERTY_NAME", "PROPERTY_VALUE");

go

create index "LIB_PROPERTIES_VER_IX2" on "LIB_PROPERTIES_VER" ("PROPERTY_NAME", "PROPERTY_VALUE", "VERSION_ID");
...
create table "LIB_PROPERTIES_NONVER" ( 
	...
	"PROPERTY_VALUE" nvarchar(256) not null,
	"PROPERTY_BLOB_VALUE" varbinary(max) null,
	constraint "FK_LIB_PROPERTIES_NONVER_FK1" foreign key ("ITEM_ID")
	 	references "LIB_CURRENT_ITEMS" ("ITEM_ID") on update no action on delete cascade
)

go

create index "LIB_PROPERTIES_NONVER_IX1" on "LIB_PROPERTIES_NONVER" ("ITEM_ID", "PROPERTY_NAME", "PROPERTY_VALUE");

go

create index "LIB_PROPERTIES_NONVER_IX2" on "LIB_PROPERTIES_NONVER" ("PROPERTY_NAME", "PROPERTY_VALUE", "ITEM_ID");
...
create view "LIB_PROPERTIES" as 
  ...
  from 
	LIB_CURRENT_ITEMS lci 
	inner join LIB_PROPERTIES_VER lp 
		on lci.VERSION_ID = lp.VERSION_ID
  union all
  select 
	ITEM_ID, 
	PROPERTY_NAME, 
...
create table "LIB_ACCESS" ( 
	...
	foreign key ("ITEM_ID")
	 	references "LIB_CURRENT_ITEMS" ("ITEM_ID") 
...
create table "LIB_RESOLVED_DEPEND_VER" (
	"DEPENDENT_VERSION_ID" char(36) not null,
  	...
  	primary key ("DEPENDENT_VERSION_ID", "REQUIRED_ID"),
  	foreign key ("DEPENDENT_VERSION_ID")
...
create view "LIB_RESOLVED_DEPEND" as 
  ...
  from 
	LIB_CURRENT_ITEMS lci 
	inner join LIB_RESOLVED_DEPEND_VER lrd 
		on lci.VERSION_ID = lrd.DEPENDENT_VERSION_ID
  
go

create index "LIB_RESOLVED_INDEX1" 
	on "LIB_RESOLVED_DEPEND_VER" ("DEPENDENT_VERSION_ID")  
	
go

create index "LIB_RESOLVED_INDEX2" 
	on "LIB_RESOLVED_DEPEND_VER" ("REQUIRED_ID")  
...
create view "LIB_UNRESOLVED_DEPEND" as 
  ...
  from 
	LIB_CURRENT_ITEMS lci 
	inner join LIB_UNRESOLVED_DEPEND_VER lud 
		on lci.VERSION_ID = lud.DEPENDENT_VERSION_ID
  
go

create index "LIB_UNRESOLVED_INDEX1" 
	on "LIB_UNRESOLVED_DEPEND_VER" ("DEPENDENT_VERSION_ID")  
	
go

create index "LIB_UNRESOLVED_INDEX2" 
	on "LIB_UNRESOLVED_DEPEND_VER" ("REQUIRED_ID")  
...
create procedure usp_copyItem 
  @oldId char(36), 
  @oldVersionId char(36),
  @newId char(36), 
  @newVersionIdSuggestion char(36), 
  @parentId char(36), 
  @caller char(36), 
...
begin 
	...
	declare @sourceVersionId char(36);
	if (@oldVersionId is null)
	begin
		set @sourceVersionId = (select VERSION_ID from LIB_CURRENT_ITEMS where ITEM_ID = @oldId);
	end
	else
	begin
		-- We should copy the given version
		declare @cnt int;
		set @cnt = (select count(1) from LIB_ITEM_VERSIONS where ITEM_ID = @oldId and VERSION_ID = @oldVersionId);
		if(@cnt =0)
		begin
			RAISERROR ('ERR-50007 - Version does not exist', 16, 1)
			ROLLBACK TRANSACTION
		end
		
		set @sourceVersionId = @oldVersionId;
	end
	
	declare @targetVersionId char(36);
	if ( @sourceVersionId is not null )
	begin
		set @targetVersionId = @newVersionIdSuggestion
  ...
  insert into LIB_CURRENT_ITEMS (ITEM_ID, VERSION_ID, TITLE, DESCRIPTION, ITEM_TYPE, CREATED_BY, CREATED, MODIFIED_BY, MODIFIED, PARENT_ID, HIDDEN)
    select 
		@newId as ITEM_ID, 
		@targetVersionId as VERSION_ID, 
	...
	insert into #LIB_COPY_MAPPING (ORIGINAL_ITEM_ID, ORIGINAL_VERSION_ID, COPY_ITEM_ID, COPY_VERSION_ID) 
	values (@oldId, @sourceVersionId, @newId, @targetVersionId); 
...
begin
  ...
  -- Properties
  insert into LIB_PROPERTIES_NONVER (ITEM_ID, PROPERTY_NAME, PROPERTY_VALUE, PROPERTY_BLOB_VALUE)
    select cm.COPY_ITEM_ID as ITEM_ID, PROPERTY_NAME, PROPERTY_VALUE, PROPERTY_BLOB_VALUE
  ...
  insert into LIB_PROPERTIES_VER (VERSION_ID, PROPERTY_NAME, PROPERTY_VALUE, PROPERTY_BLOB_VALUE)
    select cm.COPY_VERSION_ID as VERSION_ID, PROPERTY_NAME, PROPERTY_VALUE, PROPERTY_BLOB_VALUE
  ...
  insert into LIB_ACCESS (ITEM_ID, USER_ID, GROUP_ID, PERMISSION)
    select cm.COPY_ITEM_ID as ITEM_ID, USER_ID, GROUP_ID, PERMISSION
    from LIB_ACCESS original, #LIB_COPY_MAPPING cm
    where original.ITEM_ID = cm.ORIGINAL_ITEM_ID;

  -- Words
  insert into LIB_WORDS (ITEM_ID, PROPERTY, WORD)
    select cm.COPY_ITEM_ID as ITEM_ID, PROPERTY, WORD
  ...
  insert into LIB_UNRESOLVED_DEPEND_VER (DEPENDENT_VERSION_ID, REQUIRED_ID, DESCRIPTION, CASCADING_DELETE, ORIGINAL_REQUIRED_ID)
    select cm.COPY_VERSION_ID as DEPENDENT_VERSION_ID, REQUIRED_ID, DESCRIPTION, CASCADING_DELETE, ORIGINAL_REQUIRED_ID
  ...
  insert into LIB_RESOLVED_DEPEND_VER (DEPENDENT_VERSION_ID, REQUIRED_ID, DESCRIPTION, CASCADING_DELETE, ORIGINAL_REQUIRED_ID)
    ...
    where original.DEPENDENT_VERSION_ID = dep.ORIGINAL_VERSION_ID
    and not exists (select 1 from #LIB_COPY_MAPPING where ORIGINAL_VERSION_ID = original.REQUIRED_ID);
	
	-- Return all copied items
    select i.ITEM_ID, i.TITLE, i.DESCRIPTION, i.ITEM_TYPE, i.FORMAT_VERSION, i.CREATED_BY, i.CREATED, 
		i.MODIFIED_BY, i.MODIFIED, i.ACCESSED, i.CONTENT_SIZE, i.PARENT_ID, i.HIDDEN, i.VERSION_ID, i.DATA_ID
		from LIB_ITEMS i, #LIB_COPY_MAPPING cm where i.ITEM_ID = cm.COPY_ITEM_ID;
...
begin
	...
	if (@newTitle is null)
		update LIB_CURRENT_ITEMS set MODIFIED_BY = @caller, MODIFIED = @timeOfUpdate,  PARENT_ID = @newParentId 
			where ITEM_ID = @itemId;
	else
		update LIB_CURRENT_ITEMS set TITLE = @newTitle, MODIFIED_BY = @caller, MODIFIED = @timeOfUpdate, PARENT_ID = @newParentId 
			where ITEM_ID = @itemId;
end;
...
begin
	...
	if (@administrationEnabled = 1) and exists (select 1 from @GROUP_MEMBERSHIPS 
		...
		begin
			...
			(
				select ITEM_ID as PARENT_ID from LIB_CURRENT_ITEMS where ITEM_ID = @itemId
				union all
				select i.PARENT_ID from PERMISSIONS_ON_ITEM poi, LIB_CURRENT_ITEMS i
...
begin
  ...
  declare c1 cursor local fast_forward for
    ...
    (
      ...
      from LIB_CURRENT_ITEMS item, CTE parent 
  ...
  begin
    ...
    with PERMISSIONS_ON_ITEM as
      (
        select ITEM_ID as PARENT_ID from LIB_CURRENT_ITEMS where ITEM_ID = @descendant
        union all
        select i.PARENT_ID from PERMISSIONS_ON_ITEM poi, LIB_CURRENT_ITEMS i
        where poi.PARENT_ID = i.ITEM_ID
        and not exists (select 1 from LIB_ACCESS acl where acl.ITEM_ID = poi.PARENT_ID)
...
create procedure usp_insertDependency
  @dependentId char(36), 
  @requiredId char(36),
...
begin
	...
	if exists (select 1 from LIB_CURRENT_ITEMS where ITEM_ID = @requiredId)
		insert into LIB_RESOLVED_DEPEND_VER (DEPENDENT_VERSION_ID, REQUIRED_ID, DESCRIPTION, CASCADING_DELETE, ORIGINAL_REQUIRED_ID) 
...
begin 
	...
	if not exists (select 1 from LIB_CURRENT_ITEMS where ITEM_ID = @newParentId) 
		raiserror('ERR-50005 - The item with ID %s does not exist.', 16, 1, @newParentId); 

	-- Verify that the parent is a container item 
	if exists (select 1 from LIB_ITEM_TYPES it, LIB_CURRENT_ITEMS i where i.ITEM_TYPE = it.TYPE_ID 
	...
	if exists (select 1 from LIB_CURRENT_ITEMS where ITEM_ID = @newItemId) 
	...
	if exists (select 1 from LIB_CURRENT_ITEMS where TITLE = @newTitle and ITEM_TYPE = @newItemType and 
	...
	with CTE (LVL, ITEM_ID) as 
		(select 1 as LVL,  PARENT_ID from LIB_CURRENT_ITEMS where ITEM_ID = @newParentId 
		union all 
		select cte.LVL + 1, i.PARENT_ID from LIB_CURRENT_ITEMS i, CTE cte 
	...
	begin
		...
		insert into LIB_ITEM_VERSIONS (VERSION_ID, ITEM_ID, DATA_ID, FORMAT_VERSION, CREATED_BY, CREATED)
		  values (@newVersionId, @newItemId, @newDataId, @newFormatVersion, @caller, @timeOfInsertion);
	end
	
	-- Insert the item
	insert into LIB_CURRENT_ITEMS (ITEM_ID, VERSION_ID, TITLE, DESCRIPTION, ITEM_TYPE, CREATED_BY, 
	  CREATED, MODIFIED_BY, MODIFIED, PARENT_ID, HIDDEN)
	values (@newItemId, @newVersionId, @newTitle, @newDescription, @newItemType, @caller, 
		@timeOfInsertion, @caller, @timeOfInsertion, @newParentId, @newHidden);

	-- Move any unresolved dependencies upon the inserted item into the set of resolved dependencies
	insert into LIB_RESOLVED_DEPEND_VER (DEPENDENT_VERSION_ID, REQUIRED_ID, DESCRIPTION, CASCADING_DELETE, ORIGINAL_REQUIRED_ID)
	(
		select DEPENDENT_VERSION_ID, REQUIRED_ID, DESCRIPTION, CASCADING_DELETE, ORIGINAL_REQUIRED_ID
		from 
			LIB_UNRESOLVED_DEPEND_VER
		where 
			REQUIRED_ID = @newItemId
	);
	
	delete from LIB_UNRESOLVED_DEPEND_VER where REQUIRED_ID = @newItemId;	
end;
...
begin 
  ...
  if not exists (select 1 from LIB_CURRENT_ITEMS where ITEM_ID = @itemId) 
  ...
  select @parentId = PARENT_ID, @itemType = ITEM_TYPE, @timeOfUpdate = convert(varchar(23), getutcdate(), 121) 
    from LIB_CURRENT_ITEMS where ITEM_ID = @itemId; 

  -- Verify that the title-type-parent combination is unique 
  if exists (select 1 from LIB_CURRENT_ITEMS where TITLE = @newTitle and ITEM_TYPE = @itemType and 
  ...
  begin
    insert into LIB_ITEM_VERSIONS (VERSION_ID, ITEM_ID, DATA_ID, FORMAT_VERSION, CREATED_BY, CREATED)
      values (@versionId, @itemId, @newDataId, @newFormatVersion, @caller, @timeOfUpdate);
	  
    update LIB_CURRENT_ITEMS set VERSION_ID = @versionId, TITLE = @newTitle, DESCRIPTION = @newDescription,
      MODIFIED_BY = @caller, MODIFIED = @timeOfUpdate, HIDDEN = @newHidden where ITEM_ID = @itemId;
  end
  else
  begin
    update LIB_CURRENT_ITEMS set TITLE = @newTitle, DESCRIPTION = @newDescription,
      MODIFIED_BY = @caller, MODIFIED = @timeOfUpdate, HIDDEN = @newHidden where ITEM_ID = @itemId;
  end
...
begin
	...
	-- Save the parent id
	declare @parentId char(36);
	set @parentId = (select PARENT_ID from LIB_CURRENT_ITEMS where ITEM_ID = @itemId);
	if (@parentId is null)
	...
	delete from LIB_RESOLVED_DEPEND_VER where REQUIRED_ID in (select DESCENDANT_ID from @descendants);

	-- Check if not analysisstate
	declare @notAnalysisState int
	set @notAnalysisState = (
		select count(1) 
	      from LIB_CURRENT_ITEMS
	      where ITEM_ID = @itemId
	...
	delete from LIB_CURRENT_ITEMS where ITEM_ID in (select DESCENDANT_ID from @descendants);

	-- Keep the data ID of all deleted versions
	insert into #descendants_data (DATA_ID) (select DATA_ID from LIB_ITEM_VERSIONS where ITEM_ID in
	(select DESCENDANT_ID from @descendants));

	-- Delete all versions of the item itself and any descendants
	delete from LIB_ITEM_VERSIONS where ITEM_ID in
	(select DESCENDANT_ID from @descendants);

	-- Touch the parent, if not analysisstate
	if (@notAnalysisState > 0)
	BEGIN
		declare @timeOfUpdate datetime;
		set @timeOfUpdate = (select convert(varchar(23), getutcdate(), 121));
		update LIB_CURRENT_ITEMS set MODIFIED_BY = @caller, MODIFIED = @timeOfUpdate where ITEM_ID = @parentId;
	END
end;
...
begin
  ...
  -- Delete all data that is no longer referenced
  delete from LIB_DATA where DATA_ID in
    (select DATA_ID from #descendants_data 
      where not exists (select 1 from LIB_ITEM_VERSIONS where LIB_ITEM_VERSIONS.DATA_ID = LIB_DATA.DATA_ID));

  -- Return the items that have been deleted
  select DATA_ID from #descendants_data
  where not exists (select 1 from LIB_DATA where LIB_DATA.DATA_ID = #descendants_data.DATA_ID);

  -- Drop the temporary tables
  drop table #descendants;
  drop table #descendants_data;
end;

go

create procedure usp_restoreVersion (
  @itemId char(36),
  @versionId char(36),
  @newVersionId char(36),
  @caller char(36))
...
begin
  ...
  select @newVersionId as VERSION_ID, ITEM_ID, DATA_ID, FORMAT_VERSION, @caller as CREATED_BY, @timeOfUpdate as CREATED, 
    VERSION_ID as RESTORED_FROM_ID, CREATED as RESTORED_FROM_TIMESTAMP
  from LIB_ITEM_VERSIONS where VERSION_ID = @versionId and ITEM_ID = @itemId;
  if (@@ROWCOUNT = 0) 
  begin
    raiserror ('ERR-50007 - The version does not exist', 16, 1)
  end
  ...
  update LIB_CURRENT_ITEMS set VERSION_ID = @newVersionId, MODIFIED_BY = @caller, MODIFIED = @timeOfUpdate 
    where ITEM_ID = @itemId;
  if (@@ROWCOUNT = 0) 
  begin
    raiserror ('ERR-50005 - The item does not exist.', 16, 1);
  end
end;

go

create procedure usp_deleteVersion 
  @itemId char(36), 
  @versionId char(36),
  @caller char(36)
as
begin
  ...
  declare @itemIdOfVersion char(36);
  set @itemIdOfVersion = (select ITEM_ID from LIB_ITEM_VERSIONS where VERSION_ID = @versionId);
  if (@itemIdOfVersion is null)
    -- The version does not exist (which is what we wanted to accomplish)
    return;
  
  if (@itemId != @itemIdOfVersion) 
    -- Trying to delete a version that belongs to another item than the given one
    raiserror('ERR-50005 - The item with ID %s does not exist.', 16, 1, @itemId); 

  -- Check if we're deleting the current version
  declare @currentVersionId char(36);
  set @currentVersionId = (select VERSION_ID from LIB_CURRENT_ITEMS where ITEM_ID = @itemId);
  if (@versionId = @currentVersionId)
  begin
    declare @previousVersionId char(36);
    set @previousVersionId = (select top(1) VERSION_ID from LIB_ITEM_VERSIONS 
      where ITEM_ID = @itemId
      and CREATED < (select CREATED from LIB_ITEM_VERSIONS where VERSION_ID = @versionId)
      order by CREATED desc);

    if (@previousVersionId is null)
      raiserror('ERR-50004 - Illegal argument. %s', 16, 1, 'Cannot delete the last version.');

    update LIB_CURRENT_ITEMS set VERSION_ID = @previousVersionId, 
      MODIFIED_BY = @caller, MODIFIED = convert(varchar(23), getutcdate(), 121) where ITEM_ID = @itemId;
  end;

  -- Keep the data ID of the deleted version
  insert into #descendants_data (DATA_ID) (select DATA_ID from LIB_ITEM_VERSIONS where ITEM_ID = @itemId and VERSION_ID = @versionId);
...
create table "CONSENTED_ENTITIES" (
   ...
   "CONSENTED_ENTITY" varbinary(max) not null,
   "ADDED_DATE" datetime null,
...
create index "CONSENTED_ENTITES_IX1" on "CONSENTED_ENTITIES" ("CONSENTING_USER_ID")
go

create index "CONSENTED_ENTITES_IX2" on "CONSENTED_ENTITIES" ("LIBRARY_ITEM_ID")
go
...
alter table ROUTING_RULES  add constraint FK_ROUT_RULES_LIB_ITEMS foreign key(LIB_ITEM_ID)references LIB_CURRENT_ITEMS(ITEM_ID)
on delete set null
go
